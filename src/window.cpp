...
void Window::on_btnRotate_clicked()
{
    ...
    bool ok1, ok2, ok3;
    double xDeg = ui.txtRotX->text().toDouble(&ok1);
    double yDeg = ui.txtRotY->text().toDouble(&ok2);
    double zDeg = ui.txtRotZ->text().toDouble(&ok3);
...
void Window::on_btnPivot_clicked()
{
    // Get info from the textboxes
    bool ok1, ok2, ok3;
    double xPos = ui.txtPivotX->text().toDouble(&ok1);
    double yPos = ui.txtPivotY->text().toDouble(&ok2);
    double zPos = ui.txtPivotZ->text().toDouble(&ok3);
    // If some data couldn't be converted to double, show error message and do nothing
    if (!(ok1 && ok2 && ok3))
    {
        showErrorMessage("Error with the format of the data.");
    ...
    // Rotate the shape and repaint the widget
    ui.renderArea->setPivot(xPos, yPos, zPos);
}
